...
export function build() {
	...
	const post_build_directory = path.join(process.cwd(), config.build_path, 'post')
	const tags_build_directory = path.join(process.cwd(), config.build_path, 'tag')
	const authors_build_directory = path.join(process.cwd(), config.build_path, 'user', 'posts')
	...
	if (!fs.existsSync(post_build_directory)) {
		fs.mkdirSync(post_build_directory, { recursive: true })
	}
	...
	for (const item of post_list) {
		...
		fs.writeFileSync(
			path.join(post_build_directory, `${post.slug}.json`),
	...
	if (!fs.existsSync(tags_build_directory)) {
		fs.mkdirSync(tags_build_directory, { recursive: true })
	}
	...
	for (const tag of Object.keys(tags)) {
		...
		fs.writeFileSync(
			path.join(tags_build_directory, `${tag_slug}.json`),
			JSON.stringify(tags[tag], null, 4)
		)
	}
	console.log()

	if (!fs.existsSync(authors_build_directory)) {
		fs.mkdirSync(authors_build_directory, { recursive: true })
	}
	...
	for (const author of Object.keys(authors)) {
		...
		fs.writeFileSync(
			path.join(authors_build_directory, `${author}.json`),
	...
	console.log('5')
	fs.writeFileSync(
		path.join(build_directory, 'posts.5.json'),
		JSON.stringify(index.posts.slice(0, 5), null, 4)
	)
	console.log('10')
	fs.writeFileSync(
		path.join(build_directory, 'posts.10.json'),
		JSON.stringify(index.posts.slice(0, 10), null, 4)
	)
	console.log(`All (${index.posts.length})`)
	fs.writeFileSync(path.join(build_directory, 'posts.json'), JSON.stringify(index.posts, null, 4))
	console.log()

	console.log('Writing Tag Index...')
	fs.writeFileSync(path.join(build_directory, 'tags.json'), JSON.stringify(index.tags, null, 4))
	console.log()
}
