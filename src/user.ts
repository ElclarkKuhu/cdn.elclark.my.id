...
export function build() {
	...
	const user_build_directory = path.join(process.cwd(), config.build_path, 'user')
	...
	if (!fs.existsSync(user_build_directory)) {
		fs.mkdirSync(user_build_directory, { recursive: true })
	}
	...
	for (const item of user_list) {
		...
		fs.writeFileSync(
			path.join(user_build_directory, `${user.username}.json`),
	...
	fs.writeFileSync(path.join(build_directory, 'users.json'), JSON.stringify(index, null, 4))
	console.log()
}
